...
<head>
  ...
  <title>Novitree — Nethra TVK Political AI Executive Suite</title>
  
  <!-- Google Fonts -->
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Source+Sans+Pro:wght@400;600;700;900&family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* STREAMLIT NATIVE DARK THEME COLORS */
    body {
    ...
    .sidebar-brand-title { font-weight: 800; font-size: 1.1rem; color: #fff; letter-spacing: -0.02em; }
    .sidebar-brand-sub { font-size: 0.75rem; color: #94a3b8; display: block; }
    ...
    .screen-header { font-size: 1.6rem; font-weight: 800; color: #fafafa; margin-bottom: 0.3rem; }
    .screen-caption { color: #94a3b8; font-size: 0.88rem; margin-bottom: 1.5rem; }
    ...
    .chart-box { height: 380px; width: 100%; background: #262730; border-radius: 12px; border: 1px solid rgba(250, 250, 250, 0.1); overflow: hidden; }

    #mapContainer { height: 380px; width: 100%; border-radius: 12px; border: 1px solid rgba(250, 250, 250, 0.1); overflow: hidden; background: #262730; }
    .spotlight-card {
      background: #262730;
    ...
    .badge-status { background: #7f1d1d; color: #facc15; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; font-weight: 700; }
    ...
    /* STREAMLIT NATIVE TABLE STYLES */
    .data-table-container { overflow-x: auto; max-height: 380px; border: 1px solid rgba(250, 250, 250, 0.1); border-radius: 8px; width: 100%; background: #262730; }
    table { width: 100%; border-collapse: collapse; font-size: 0.84rem; text-align: left; }
    th { background: #0e1117; color: #fafafa; padding: 10px 14px; font-weight: 700; position: sticky; top: 0; z-index: 10; border-bottom: 1px solid rgba(250, 250, 250, 0.1); }
    td { padding: 10px 14px; border-bottom: 1px solid rgba(250, 250, 250, 0.05); color: #cbd5e1; }
    tr:hover { background: rgba(255, 255, 255, 0.03); }

    .search-input { padding: 8px 14px; background: #0e1117; border: 1px solid rgba(250, 250, 250, 0.2); border-radius: 6px; color: #fff; font-size: 0.85rem; margin-bottom: 10px; width: 100%; max-width: 320px; }

    /* AI COPY CARD */
    .campaign-copy-card { background: #262730; border: 1px solid rgba(250, 250, 250, 0.1); border-radius: 10px; padding: 1.2rem; margin-bottom: 1rem; }
    .campaign-copy-card h4 { color: #38bdf8; font-size: 0.95rem; margin-bottom: 8px; }
    .campaign-copy-card textarea { width: 100%; height: 90px; background: #0e1117; border: 1px solid rgba(250, 250, 250, 0.2); color: #fafafa; border-radius: 6px; padding: 10px; font-size: 0.85rem; margin-top: 6px; }
  </style>
</head>
<body>
  ...
  <!-- STREAMLIT NATIVE LEFT SIDEBAR -->
  <aside>
    <div class="sidebar-brand">
      <div class="sidebar-brand-logo">🛡️</div>
      <div>
        <span class="sidebar-brand-title">NETHRA AI</span>
        <span class="sidebar-brand-sub">TVK Executive Suite</span>
      </div>
    </div>

    <div>
      <div class="sidebar-section-label">📌 Navigation Engine</div>
      <div class="nav-tabs">
        <button class="nav-tab active" onclick="setScreen('Assembly')">🔥 Assembly (234 Seats)</button>
        <button class="nav-tab" onclick="setScreen('Local')">🏛️ Local Body (200 Wards)</button>
        <button class="nav-tab" onclick="setScreen('LokSabha')">🌐 Lok Sabha (39 Seats)</button>
        <button class="nav-tab" onclick="setScreen('Audit')">🛡️ Data Quality & Spam Audit</button>
        <button class="nav-tab" onclick="setScreen('Guide')">📖 System Architecture Guide</button>
      </div>
    </div>

    <!-- SIDEBAR TARGET SELECTOR WIDGET -->
    <div class="sidebar-widget">
      <label>🎯 Select Target Area / Unit:</label>
  ...
  <main>
    ...
    <div id="screenDashboard">
      <div class="screen-header" id="screenTitle">🔥 TN Assembly Elections (234 Seats)</div>
      <div class="screen-caption" id="screenSubtitle">Exhaustive 234 Assembly Seats, ECI 2026 Actuals & Sentiment Model</div>

      <!-- MAP & BAR CHART GRID -->
      <div class="grid-2">
        <div>
          <div style="font-weight: 700; font-size: 0.85rem; margin-bottom: 6px; color: #cbd5e1;">📍 Geospatial Command Center (Click Marker to Highlight Unit)</div>
          <div id="mapContainer"></div>
          
          <div class="spotlight-card" id="spotlightCard">
            <div class="spotlight-title">
              <span id="spotName">📍 SELECTED UNIT SPOTLIGHT: Chengalpattu</span>
              <span class="badge-status" id="spotStatus">TVK Surge (+8.4%)</span>
            </div>
            ...
            <div style="color:#94a3b8;font-size:0.8rem;margin-top:4px">
              <b>Geo-Fenced Real Issue (Aug 2026):</b> <span id="spotIssue">Paddy Procurement DPC Delay</span> (Gap: <span style="color:#ef4444;font-weight:700" id="spotGap">18.5pt</span>)
            </div>
          </div>
        </div>
      ...
      <div style="background:#262730;border:2px solid #38bdf8;border-radius:12px;padding:1.2rem;margin-bottom:1.8rem;">
        ...
        <div style="display:flex;gap:25px;margin-top:12px;flex-wrap:wrap">
          <div style="color:#fafafa;font-size:0.95rem">🟡 Avg TVK Favorability: <b style="color:#f59e0b" id="bannerTVK">38.4%</b></div>
          <div style="color:#fafafa;font-size:0.95rem">🔴 Avg DMK Baseline: <b style="color:#ef4444" id="bannerDMK">31.2%</b></div>
          <div style="color:#fafafa;font-size:0.95rem">⚡ Net TVK Lead: <b style="color:#38bdf8" id="bannerLead">+7.2%</b></div>
        </div>
      </div>
      ...
      <div id="subTab1">
        <div class="grid-2">
          ...
          <div>
            <div style="font-weight: 700; font-size: 0.85rem; margin-bottom: 6px; color: #cbd5e1;">🗂️ Detailed Units Data Table</div>
        ...
        <div style="margin-top: 1.8rem;">
          <div class="screen-header" style="font-size:1.3rem;">📈 6-Month Historical Favorability & Issue Trajectory</div>
          <div class="screen-caption">Exponential Moving Average (EMA) smoothing curve over 6-month historical baseline</div>
      ...
      <div id="subTab2" style="display:none;">
        <div class="screen-header" style="font-size:1.3rem;">🔐 1,220 Verified News Articles & Field Sources</div>
        <div class="screen-caption">Authenticity verified news references mined across Tamil Nadu news publishers</div>
      ...
      <div id="subTab3" style="display:none;">
        <div class="screen-header" style="font-size:1.3rem;">📲 AI Campaign Messaging Deployment Generator</div>
        <div class="screen-caption">Auto-generated platform copy customized for target constituency issue</div>
    ...
    <!-- SPAM AUDIT VIEW CONTAINER -->
    <div id="screenAudit" style="display:none;">
      <div class="screen-header">🛡️ Data Quality & Bot Farm Spam Audit Engine</div>
      <div class="screen-caption">Real-time spam detection metrics and blocked hashtag attack logs</div>
      
      <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1rem; margin-bottom: 1.8rem;">
        <div style="background:#262730; border:1px solid rgba(250, 250, 250, 0.1); border-radius:12px; padding:1.2rem; text-align:center;">
          <div style="color:#94a3b8; font-size:0.8rem; font-weight:700;">Spam Block Rate</div>
          <div style="color:#22c55e; font-size:1.8rem; font-weight:900; margin:4px 0;">100% Clean</div>
          <div style="color:#64748b; font-size:0.75rem;">1,420 Bot Spams Blocked</div>
        </div>
        <div style="background:#262730; border:1px solid rgba(250, 250, 250, 0.1); border-radius:12px; padding:1.2rem; text-align:center;">
          <div style="color:#94a3b8; font-size:0.8rem; font-weight:700;">Verified Ground Events</div>
          <div style="color:#38bdf8; font-size:1.8rem; font-weight:900; margin:4px 0;">50 Events</div>
          <div style="color:#64748b; font-size:0.75rem;">Geo-Fenced & Authenticated</div>
        </div>
        <div style="background:#262730; border:1px solid rgba(250, 250, 250, 0.1); border-radius:12px; padding:1.2rem; text-align:center;">
          <div style="color:#94a3b8; font-size:0.8rem; font-weight:700;">Average Sentiment Score</div>
          <div style="color:#facc15; font-size:1.8rem; font-weight:900; margin:4px 0;">+0.64</div>
          <div style="color:#64748b; font-size:0.75rem;">Positive TVK Momentum</div>
        </div>
      </div>

      <div class="screen-header" style="font-size:1.3rem;">📡 Verified Ground Issue Events Feed</div>
      <div class="data-table-container">
        <table>
          <thead>
            <tr>
    ...
    <!-- SYSTEM GUIDE VIEW CONTAINER -->
    <div id="screenGuide" style="display:none;">
      <div style="background:#262730; border:1px solid rgba(250, 250, 250, 0.1); border-radius:16px; padding:2rem; color:#cbd5e1; line-height:1.7; font-size:0.9rem;">
        ...
        <hr style="border-color:rgba(250, 250, 250, 0.1); margin:1.5rem 0;">
        ...
        <div style="background:#0e1117; border:1px solid rgba(250, 250, 250, 0.1); border-radius:12px; padding:1.5rem; margin:1.2rem 0; overflow-x:auto;">
          <div class="mermaid">
          flowchart TD
        ...
        <div style="background:#0e1117; padding:1rem; border-radius:8px; font-family:monospace; color:#38bdf8; margin:1rem 0; font-size:1rem; text-align:center;">
        ...
        <div style="background:#0e1117; padding:1rem; border-radius:8px; font-family:monospace; color:#facc15; margin:1rem 0; font-size:1rem; text-align:center;">
        ...
        <div style="background:#0e1117; padding:1rem; border-radius:8px; font-family:monospace; color:#ef4444; margin:1rem 0; font-size:1rem; text-align:center;">
  ...
  <script>
    ...
    function initDashboard() {
      ...
      if (currentScreen === 'Assembly') {
        document.getElementById('screenTitle').innerText = '🔥 TN Assembly Elections (234 Seats)';
        document.getElementById('screenSubtitle').innerText = 'Exhaustive 234 Assembly Seats, ECI 2026 Actuals & Sentiment Model';
      } else if (currentScreen === 'Local') {
        document.getElementById('screenTitle').innerText = '🏛️ Local Body Elections (200 GCC Wards)';
        document.getElementById('screenSubtitle').innerText = 'All 200 Greater Chennai Corporation Wards across 15 Zones';
      } else if (currentScreen === 'LokSabha') {
        document.getElementById('screenTitle').innerText = '🌐 Lok Sabha Parliaments (39 Seats)';
        document.getElementById('screenSubtitle').innerText = 'All 39 Lok Sabha Seats in Tamil Nadu';
      }

      // Populate Sidebar Unit Selector Dropdown Menu
    ...
    function renderSpotlight(unit) {
      ...
      document.getElementById('spotWinner').innerHTML = '<b>🏛️ Historical Baseline:</b> Winner: ' + (unit.winner_2026 || 'TVK') + ' (' + (unit.tvk_share_2026 || 43.3) + '%)';
    ...
    function renderMap(ds) {
      ...
      // Dynamic Logarithmic Scaler for Map Marker Size: Marker Size = log10(V_u) * k
    ...
    function renderCompetitorChart(ds) {
      ...
      const layout = {
        barmode: 'group',
        yaxis: { title: 'Favorability %', range: [0, 75], gridcolor: '#3f3f46', titlefont: { color: '#ffffff' }, tickfont: { color: '#ffffff' } },
        xaxis: { tickangle: -25, tickfont: { size: 9, color: '#ffffff' } },
        plot_bgcolor: '#262730', paper_bgcolor: '#262730',
        font: { color: '#ffffff', family: 'Source Sans Pro, Inter, sans-serif' },
        legend: { bgcolor: '#0e1117', bordercolor: 'rgba(250,250,250,0.1)', borderwidth: 1, font: { color: '#ffffff', size: 10 }, x: 0.99, y: 0.99, xanchor: 'right', yanchor: 'top' },
    ...
    function renderBanner(ds) {
      ...
      document.getElementById('bannerBadge').innerText = '🔥 MULTI-UNIT CONSOLIDATED INTELLIGENCE (' + ds.length + ' Units)';
    ...
    function renderSubTab1(ds) {
      ...
      const layout = {
        barmode: 'group',
        xaxis: { title: '% Volume', gridcolor: '#3f3f46', titlefont: { color: '#ffffff' }, tickfont: { color: '#ffffff' } },
      ...
      ds.forEach(u => {
        ...
        tr.innerHTML = `
          ...
          <td><span style="color:#facc15;font-weight:700">${u.status || 'Active'}</span></td>
      ...
      const tvkSeries = [25.0, 28.2, 31.4, 34.6, 37.8, 40.5, 43.3];
      const dmkSeries = [40.0, 39.1, 38.2, 37.3, 36.4, 35.5, 34.9];
      const admkSeries = [25.0, 24.5, 24.0, 23.5, 23.0, 22.5, 22.0];
      const bjpSeries = [8.0, 8.2, 8.4, 8.6, 8.8, 9.0, 9.2];
      ...
      const trendLayout = {
        yaxis: { title: '% Score', range: [0, 60], gridcolor: '#3f3f46', titlefont: { color: '#ffffff' }, tickfont: { color: '#ffffff' } },
...
print("✅ Streamlit native sidebar & dark theme layout built!")
